...
use events::control_x_player::ControlToPlayer;
    ...
    where F1: Send + Fn(&mut PlayerSystem<F1>, RunArg)
{
    ...
    while let Some(event) = me.get_mut_control_channel().try_recv() {
        match event {
            ...
            ControlToPlayer::Start(_player_evt) => {}
            ControlToPlayer::Select(_player_evt) => {}
        }
    }
}
